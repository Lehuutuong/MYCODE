...
void sha1Ex(unsigned char hval[], const unsigned char data[], unsigned __int32 len)
{   sha1_ctx    cx[1];
	...
	{
		sha1_hash(data + i*nLen, nLen, cx); 
	}
	int nRemain = len - (len/nLen)*nLen;
	if(nRemain)
	{
		sha1_hash(data + i*nLen, nRemain, cx); 
	}
	sha1_end(hval, cx);
}

BOOL sha1File(unsigned char hval[], LPCTSTR fileName)
{	sha1_ctx    cx[1];

	CFile file;
	char buf[4096];
	UINT nRead, nFileSize;
	UINT nSentSize = 0;
	...
	try
	{
		...
		{
			//MyOutputDebugString(DBG_LING3, DBG_ERR, L"Server : 파일계산에서 파일열기 실패(%s)", fileName);
			memset(hval, 0, 20);
			return FALSE;
		}
		...
		while((nRead = file.Read(buf, 4096)) > 0)
		{
			sha1_hash((const unsigned char*)buf, nRead, cx); 
			nSentSize += nRead;
		}
